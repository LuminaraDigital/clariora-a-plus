...
| File | Purpose |
|---|---|
| `rate-limit-ruleset.json` | The rules, with a `_rationale` on each one. Source of truth. |
| `deploy-rate-limits.mjs` | Validates and PUTs the ruleset to the zone entrypoint. |
...
The deploy script refuses to send a ruleset that would silently misbehave: an
unsupported `period`, missing `cf.colo.id` in the characteristics, or a specific rule
ordered *below* the `/api/` catch-all, where it could never be the first rule to block.
