...
def check_intro_gate(res: Results) -> None:
    key = "aplus3_boot_intro_seen_v1"
    # shell-ux.js is the documented home for the gate. boot-intro.js and index.html
    # are regenerated by a separate workstream, so either of them also satisfies it.
    candidates = [
        ROOT / "js" / "shell-ux.js",
    ...
    for path in candidates:
        if path.is_file() and key in path.read_text(encoding="utf-8", errors="replace"):
            hits.append(path.relative_to(ROOT).as_posix())
    if hits:
        res.ok("intro gate key", ", ".join(hits))
    else:
        res.fail("intro gate key", f"{key} not found in shell-ux.js, boot-intro.js or index.html")
...
def check_versions(res: Results) -> None:
    ...
    if version and version == root_version == app_version:
        res.ok("version consistency", f"v{version}")
    else:
        res.fail(
            "version consistency",
            f"release.config={version} root={root_version} app={app_version}",
        )
